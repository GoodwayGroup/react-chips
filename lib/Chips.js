...
var Chips = function (_Component) {
  ...
  function Chips(props) {
    ...
    _this.addChips = function (values) {
      var values_normalized = values;
      if (_this.props.uniqueChips) {
        values_normalized = [].concat(_toConsumableArray(new Set(values))).filter(function (value) {
          return _this.props.value.indexOf(value) !== -1;
        });
      }

      var chips = [].concat(_toConsumableArray(_this.props.value), _toConsumableArray(values_normalized));
      _this.props.onChange(chips);
      _this.setState({ value: "" });
    };
